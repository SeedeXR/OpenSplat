...
void Model::afterTrain(int step){
    ...
    if (step % refineEvery == 0 && step > warmupLength){
        ...
        // Resource-aware hard cap (process contract / --max-splats): stop GROWING
        // gaussians once the VRAM budget is reached, but keep culling so the count
        // can still shrink. 0 => unbounded.
