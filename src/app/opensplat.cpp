...
int main(int argc, char *argv[]){
    ...
    options.add_options()
        ...
        ("max-splats", "Soft cap on gaussian count to bound VRAM: pauses densification once exceeded; final count may overshoot by up to one densification batch (0 = unbounded)", cxxopts::value<long long>()->default_value("0"))
    ...
    try{
        ...
        // detectHardware only knows CUDA-vs-not; label the actual compute backend so the
        // contract banner doesn't say "CPU-only" on a Metal/MPS run (uniform across backends).
        if (device == torch::kMPS) hw.accelerator = "Metal/MPS";
        else if (device == torch::kCPU) hw.accelerator = "CPU-only";
        ...
        if (device == torch::kMPS){
            ...
            std::cout << "MPS memory: current allocated "
                      << mps.getCurrentAllocatedMemory() / (1024 * 1024) << " MB, driver allocated "
                      << mps.getDriverAllocatedMemory() / (1024 * 1024) << " MB" << std::endl;
        }
